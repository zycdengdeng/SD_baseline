...
from src.dataset.Matterport3D_test import MP3Ddataset as MP3Ddataset_test
...
def parse_args():
    ...
    parser.add_argument(
        '--skip_first_n', type=int, default=0,
        help='skip first N samples from dataset')
...
if __name__ == "__main__":
    ...
    mode = 'train' if args.eval_on_train else 'test'

    if config['dataset']['name'] == 'mp3d':
        dataset = MP3Ddataset(
            config['dataset'], mode=mode)
    elif config['dataset']['name'] == 'mp3d_test':
        dataset = MP3Ddataset_test(
            config['dataset'], mode=mode)
