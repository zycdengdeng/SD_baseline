...
class PanoOutpaintGenerator(pl.LightningModule):
    ...
    def prepare_depth_latents(self, batch, height, width):
        if 'depth_inv_norm_small' in batch:
            depth = batch['depth_inv_norm_small']  
            if depth.dim() == 4:
                depth = depth[:, :, None, :, :]     
            return depth.to(dtype=torch.float32, device=batch['images'].device)

        elif 'depth_inv_norm' in batch:
          
            depth_full = batch['depth_inv_norm'].to(dtype=torch.float32, device=batch['images'].device)
            if depth_full.dim() == 4:
                depth_full = depth_full[:, :, None, :, :]  # [bs, m, 1, H, W]
            bs, m, _, H, W = depth_full.shape
            depth_small = F.interpolate(depth_full.reshape(bs*m, 1, H, W),
                                        size=(height // 8, width // 8),
                                        mode='bilinear', align_corners=False)
            depth_small = depth_small.reshape(bs, m, 1, height // 8, width // 8)
            return depth_small
        else:
            raise KeyError("Depth conditioning requested, but neither 'depth_inv_norm_small' nor 'depth_inv_norm' found in batch.")
    ...
    def training_step(self, batch, batch_idx):
        ...
        _, m, _, h, w = images.shape  # images is [bs, m, c, H, W]
        depth_latents = self.prepare_depth_latents(batch, h, w)  # [bs, m, 1, H/8, W/8]

        latents_input = torch.cat([noise_z, mask_latnets, masked_image_latents,depth_latents], dim=2)
    ...
    def inference(self, batch):
        ...
        depth_latents = self.prepare_depth_latents(batch, h, w)
        for i, t in enumerate(timesteps):
            _timestep = torch.cat([t[None, None]]*m, dim=1)
            latent_model_input = torch.cat([latents, mask_latnets, masked_image_latents,depth_latents], dim=2)
