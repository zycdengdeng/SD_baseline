...
class MP3Ddataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        real_prompt_path = item['real_prompt_path']
        cond_prompt_paths = item['cond_prompt_paths']
        ...
        prompt_paths = [real_prompt_path] + cond_prompt_paths
        prompts = []
        for pkl_path in prompt_paths:
            if not os.path.exists(pkl_path):
                raise FileNotFoundError(f"[ERROR] Prompt file not found: {pkl_path}")
            with open(pkl_path, "rb") as f:
                prompt = pickle.load(f)
                if not isinstance(prompt, str):
                    raise ValueError(f"[ERROR] Prompt in {pkl_path} is not a string.")
