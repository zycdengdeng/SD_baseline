...
def generate_condition_pairs_all_scenes(
    ...
    for scene_dir in tqdm(scene_dirs, desc="Processing all scenes"):
        ...
        all_files = [f for f in os.listdir(scene_dir) if f.endswith(".png")]
        angle_to_file = {}
        ...
        for real_angle in real_angles:
            ...
            for i, j in product(range(num_left), range(num_right)):
                left_path = os.path.join(scene_dir, angle_to_file[left_angles[i]])
                right_path = os.path.join(scene_dir, angle_to_file[right_angles[j]])
                real_prompt_path = os.path.splitext(real_path)[0] + ".pkl"
                cond_prompt_paths = [
                    os.path.splitext(left_path)[0] + ".pkl",
                    os.path.splitext(right_path)[0] + ".pkl"
                ]
                all_results.append({
                    'scene': scene_name,
                    'real_img_path': real_path,
                    'real_prompt_path': real_prompt_path,
                    'real_angle': real_angle,
                    'cond_img_paths': [left_path, right_path],
                    'cond_prompt_paths': cond_prompt_paths,
    ...
    # === Save separately ===
    base_path = os.path.splitext(save_path)[0]  # remove .npy
...
generate_condition_pairs_all_scenes(
    parent_dir="/mnt/vdc1/lyt/panoptic_data/extracted/panoptic_360_output",
    save_path="/mnt/vdb1/lyt/condition_pairs_new.npy",
    real_angles=[0, 55, 110, 180, -110, -55],
    fov=90
)